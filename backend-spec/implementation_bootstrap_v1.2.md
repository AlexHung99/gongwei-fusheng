...
`backend-spec/` 是規格交付來源；後端 Repository 的 `db/authoritative/v1.2/` 必須保留上述三份 SQL 原檔。Setup 腳本若缺任一份檔案必須失敗，不得再用「警告後跳過」掩蓋不完整交付。
...
- v1.2 必做命令：`grant-super-admin`、`verify-database`、`show-migration-status`。
...
```text
1. 套用 schema_v1.2.sql／等價 Initial EF Migration
2. 啟動 API
3. 站長以 LINE Login 登入一次，建立 game.users
4. 停止或維持維護模式
5. AdminCli grant-super-admin
6. 執行 seed_rules_v1.2.sql
7. 執行 seed_npcs_v1.2.sql
...
`verify-database` 至少檢查 Active Super Admin、Rules Settings、63 張 Table、28 個能力標籤、8 個地點、6 個已發布場景活動、44 個啟用場景選項、8 位已發布 NPC、必要 Index／Trigger 與 Migration ID。
...
- 比對 Migration 與 `schema_v1.2.sql` 的 Table、FK、Index、Check、Trigger 語意。
- 依正式順序執行 Bootstrap／兩份 Seed，第二次重跑保持 Idempotent。
- API OpenAPI Snapshot 與 `api_v1_v1.2.md` 已實作範圍對齊。
...
- [完整後端規格](../後端規格書_v1.2.md)
- [API Contract](./api_v1_v1.2.md)
- [LINE Login](./line_login_v1.2.md)
- [PostgreSQL Schema](./schema_v1.2.sql)
- [規則 Seed](./seed_rules_v1.2.sql)
- [NPC Seed](./seed_npcs_v1.2.sql)
