...
1. [後端規格書_v1.2.md](../後端規格書_v1.2.md)：系統邊界、非功能需求、交易、權限與驗收標準。
2. [implementation_bootstrap_v1.2.md](./implementation_bootstrap_v1.2.md)：後端不存在時，從零建立 Api／Admin／Worker／AdminCli、CI、#20～#22 的開工規格。
3. [api_v1_v1.2.md](./api_v1_v1.2.md)：HTTP Method、Path、DTO、狀態碼及錯誤碼。
4. [schema_v1.2.sql](./schema_v1.2.sql)：資料表、FK、Check、Unique、Index、Trigger 與不可變資料規則。
5. [line_login_v1.2.md](./line_login_v1.2.md)：LINE Login、Cookie Session、CSRF、CORS 與正式環境設定。
6. [rank_catalog_v1.2.md](./rank_catalog_v1.2.md)：完整位階、門檻、初始能力與俸祿。
7. [seed_rules_v1.2.sql](./seed_rules_v1.2.sql)：能力標籤、地點、位階及已確認規則的 Seed。
8. [seed_npcs_v1.2.sql](./seed_npcs_v1.2.sql)：現有 8 位 NPC 的初始已發布內容與立繪路徑；只補缺少 Code，不覆蓋 CMS 編輯。

舊版 v1.0 只供追溯，不得用來產生新的 Migration 或 OpenAPI。企劃原始文件若與 v1.2 的已確認修正衝突，以 v1.2 為準；未知規則必須維持可設定，不可自行臆測。

## 2. 交付內容摘要

- 63 張 PostgreSQL Table，包含 LINE 登入、防重放、角色、玩家歷程、場景人物／抽籤、NPC CMS、事件、宮市、庫存、繁衍、死亡、審核、Audit、Outbox 與排程。
- 246 個 Method + Path API Contract；OpenAPI 應由實作自動產生並以 Snapshot Test 防止未審核破壞性修改。
...
- 太醫院大夫、閱書院先生、奉天樓仙者及三處園苑籤池均由後台發布；玩家指定籤名，後台設定該籤增加的能力與數值，前端在結算完成前不得取得隱藏效果。

## 3. 建議建置順序

1. 若後端尚不存在，必須依 `implementation_bootstrap_v1.2.md` 自行建立完整 Solution、`GongWei.Api`、`GongWei.Admin`、`GongWei.Worker`、共用層、`GongWei.AdminCli` 與測試專案；不得因專案不存在而把任務標成阻塞。
2. 以 `schema_v1.2.sql` 建立第一版 Migration；不得在 Production 直接把整份 SQL 當成每次啟動腳本重跑。
3. 建立 LINE Login、Session、CSRF、CORS 與 RBAC；完成下方首次管理員 Bootstrap。
4. 執行 `seed_rules_v1.2.sql`，再執行 `seed_npcs_v1.2.sql`；以整合測試驗證兩份 Seed 可重跑。
5. 完成角色申請／審核、人物頁、玩家清單、數值標籤及統一歷程。
6. 完成場景人物／抽籤、NPC、事件投稿／審核／結算、經濟帳本、宮市與庫存。
7. 完成侍寢／懷孕／出生、死亡重建角色、管理調整及永久 Audit。
8. 完成 Worker、Outbox、備份還原演練、負載測試、OpenAPI Snapshot 及部署 Runbook。

## 4. 全新環境 Bootstrap

`seed_rules_v1.2.sql` 內部分 `game_settings` 需要 `created_by`，因此全新資料庫必須先建立第一位超級管理員。`GongWei.AdminCli` 是後端交付的一部分，必須由開發者依 `implementation_bootstrap_v1.2.md` 建立，不是外部前置專案。Production 不允許手動修改角色表或留下通用預設密碼。
...
5. 依序執行 `seed_rules_v1.2.sql`、`seed_npcs_v1.2.sql`。若尚未有 `super_admin`，Seeder 必須直接失敗並顯示可操作的錯誤，不可靜默略過設定。
...
- EF Core Migration 是正式版本來源；`schema_v1.2.sql` 是 Initial Migration 的可審查基準。
...
- `seed_rules_v1.2.sql` 可重跑；能力標籤與地點可依權威規則更新，場景活動／人物只補缺少 Code、不覆蓋後台 CMS 編輯；任何 Seed 都不得覆寫 Ledger、角色歷史、活動結果、事件結果、死亡、Revision 或 Audit。
- `seed_npcs_v1.2.sql` 只插入缺少的 NPC Code 與其第一版 Revision；既有 NPC 即使內容不同也不得由部署 Seed 覆寫，必須走 CMS 發布／回復流程。
