...
> Schema：[schema_v0.8.sql](./schema_v0.8.sql)
...
| `GET /events/{id}/posts` | User | `cursor,limit` | `CursorPage<EventPostDto>` | Cursor 依 `postedAt,id` |
| `POST /events/{id}/posts` | Character | `CreateEventPostRequest` | `201 EventPostDto` | Idem；`EVENT_LOCKED`、`NOT_PARTICIPANT` |
| `PATCH /events/{id}/posts/{postId}` | Character | `UpdateEventPostRequest` | `EventPostDto` | 本人且截止前；`If-Match`；保存 Revision |
| `POST /events/{id}/posts/{postId}/withdraw` | Character | `ReasonRequest?` | `EventPostDto` | Idem；保留歷史不 Hard Delete |
...
| `POST /admin/characters/{id}/economy-adjustments` | EM/SA | `EconomyAdjustmentRequest` | `EconomyAdjustmentResultDto` | Idem；超過門檻回 `202 ApprovalRequestDto` |
| `POST /admin/characters/{id}/item-grants` | EM/SA | `ItemGrantRequest` | `ItemGrantResultDto` | Idem；超過門檻需 Approval |
| `GET /admin/ledger/transactions/{id}` | EM/AUD/SA | — | `LedgerTransactionDetailDto` | — |
| `POST /admin/ledger/transactions/{id}/corrections` | EM/SA | `LedgerCorrectionRequest` | `LedgerTransactionDetailDto` | Idem；只能反向補正，不能改舊 Entry；需 Approval |
...
| `GET /admin/audit-logs` | AUD/SA | Filters | `CursorPage<AuditLogDto>` | 僅讀，不提供更新／刪除 API |
| `GET /admin/audit-logs/{id}` | AUD/SA | — | `AuditLogDetailDto` | — |
...
Approval `execute` 不接受任意 SQL、Type 名稱或自由 Payload；只能依 `actionType` 對應程式碼中已註冊的 Handler，例如 `character.death`、`economy.large_adjustment`、`admin.grant_super_admin`。
...
規則：`portraitId` 與 `playerPortraitSubmissionId` 必須且只能提供一個。`prince → male`、`consort/princess → female`；後端查立繪或上傳圖的 Owner、Role 與審核狀態。申請可引用 Pending/Approved 上傳圖，但建立正式角色前必須 Approved。姓名去除首尾空白後驗證，不接受 HTML。
...
`amount` 可正可負但不可為 0；扣除後不得小於 0。達設定門檻不直接執行，回 `202 ApprovalRequestDto`。
...
{
  "resultCode": "approved-no-pregnancy | approved-pregnancy | rejected",
...
只有 `approved-pregnancy` 需要 `dueAt`。API 必須在交易中重新檢查 `available = waiting - ongoing reservations`，不能相信申請建立時的數字。
...
合計 232 個 Method + Path 組合；實作可依 Sprint 分批，但已列出的路徑、狀態碼與 DTO 變更必須透過 OpenAPI Review。MVP 不要求第一天全部上線，`後端規格書_v0.8.md` 會標出 P0/P1/P2 實作順序。
