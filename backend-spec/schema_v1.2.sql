-- 宮闈浮生 PostgreSQL schema v1.2
-- Target: PostgreSQL 17/18
-- Canonical domains: https://miglow.vip/gongwei/ and https://gongwei-api.miglow.vip
-- v1.2 removes the cancelled main-story and relationship modules, and adds durable
...
-- Backend-editable people and draw pools attached to a map location. A parent
-- activity controls entry rules; each option stores a structured reward that the
-- API settles atomically instead of trusting values supplied by the browser.
CREATE TABLE game.location_activity_definitions (
    id                      uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    location_id             uuid NOT NULL REFERENCES game.world_locations(id) ON DELETE RESTRICT,
    code                    varchar(80) NOT NULL UNIQUE,
    display_name            varchar(100) NOT NULL,
    ...
    version                 bigint NOT NULL DEFAULT 1 CHECK (version > 0),
    CHECK (char_length(btrim(display_name)) BETWEEN 1 AND 100),
    CHECK (char_length(intro_markdown) <= 10000),
    CHECK ((status = 'published' AND published_by IS NOT NULL AND published_at IS NOT NULL)
           OR status <> 'published')
);

CREATE INDEX ix_location_activity_player_list
    ON game.location_activity_definitions(location_id, sort_order, display_name)
    WHERE status = 'published' AND is_enabled = true;

CREATE TABLE game.location_activity_options (
    id                  uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    ...
    CHECK (char_length(btrim(display_name)) BETWEEN 1 AND 100),
    CHECK (reward_payload ? 'effects' AND jsonb_typeof(reward_payload->'effects') = 'array')
);

CREATE INDEX ix_location_activity_options_active
    ON game.location_activity_options(activity_id, sort_order, display_name)
...
-- One immutable settlement per successful scene action. Failed validation is not
-- inserted here; it is returned as a typed API error. The same transaction must
-- also write character_stats, Ledger/Inventory, Chronicle, progress, Audit and Outbox.
CREATE TABLE game.location_activity_attempts (
    id                      uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    character_id            uuid NOT NULL REFERENCES game.characters(id) ON DELETE RESTRICT,
    activity_id             uuid NOT NULL REFERENCES game.location_activity_definitions(id) ON DELETE RESTRICT,
...
CREATE INDEX ix_location_activity_attempts_character_time
    ON game.location_activity_attempts(character_id, happened_at DESC, id DESC);
CREATE INDEX ix_location_activity_attempts_activity_time
    ON game.location_activity_attempts(activity_id, happened_at DESC, id DESC);
...
BEGIN
    FOREACH table_name IN ARRAY ARRAY[
        ...
        'event_post_revisions', 'event_results', 'location_activity_attempts', 'births',
...
BEGIN
    FOREACH table_name IN ARRAY ARRAY[
        ...
        'world_state', 'game_settings', 'world_locations', 'location_activity_definitions',
        'location_activity_options', 'npcs', 'event_rooms', 'event_posts',
