...
CREATE TABLE game.character_applications (
    ...
    CHECK (char_length(btrim(given_name)) BETWEEN 1 AND 30),
...
CREATE TABLE game.ranks (
    ...
    updated_at        timestamptz NOT NULL DEFAULT now(),
    version           bigint NOT NULL DEFAULT 1 CHECK (version > 0),
    UNIQUE (applies_to_role, ordinal)
);
...
CREATE TABLE game.character_stats (
    ...
    talent          smallint NOT NULL DEFAULT 0 CHECK (talent BETWEEN 0 AND 100),
    strategy        smallint NOT NULL DEFAULT 0 CHECK (strategy BETWEEN 0 AND 100),
    etiquette       smallint NOT NULL DEFAULT 0 CHECK (etiquette BETWEEN 0 AND 100),
    vitality        smallint NOT NULL DEFAULT 0 CHECK (vitality BETWEEN 0 AND 100),
    prestige        bigint NOT NULL DEFAULT 0 CHECK (prestige >= 0),
    favor           integer NOT NULL DEFAULT 0 CHECK (favor BETWEEN -1000 AND 1000),
    action_points   smallint NOT NULL DEFAULT 0 CHECK (action_points BETWEEN 0 AND 100),
    updated_at      timestamptz NOT NULL DEFAULT now(),
    version         bigint NOT NULL DEFAULT 1 CHECK (version > 0)
);
...
CREATE TABLE game.event_posts (
    ...
    status          varchar(20) NOT NULL DEFAULT 'published'
                    CHECK (status IN ('published', 'edited', 'withdrawn', 'moderated')),
...
CREATE INDEX ix_event_posts_room_feed ON game.event_posts(event_room_id, posted_at, id);

CREATE TABLE game.event_post_revisions (
    id              uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    event_post_id   uuid NOT NULL REFERENCES game.event_posts(id) ON DELETE CASCADE,
    revision_no     integer NOT NULL CHECK (revision_no > 0),
    body_markdown   text NOT NULL,
    changed_by      uuid NOT NULL REFERENCES game.users(id) ON DELETE RESTRICT,
    created_at      timestamptz NOT NULL DEFAULT now(),
...
BEGIN
    FOREACH table_name IN ARRAY ARRAY[
        ...
        'world_state', 'game_settings', 'world_locations', 'event_rooms',
