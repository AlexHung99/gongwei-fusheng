...
- 查詢條件必須獨立於表格上方，提供身份、品級、啟用狀態、建角可選與關鍵字篩選，並使用明確的「搜尋」與「清除」按鈕；不得在輸入每個字時立即重載正式資料。
- 清單以表格呈現，顯示功能、身份、位階、位號、品級、月俸、員額、建角與啟用狀態，並提供每頁 10／20／50 筆的分頁。
- 每列的「編輯」會在該列下方展開完整行內編輯表單；同一時間僅展開一筆，仍需填寫異動理由才能儲存。「檢視」進入獨立詳細頁，「停用」導向詳細頁的邏輯刪除確認區。
- RWD 在寬度 900px 以下將每列轉為雙欄資料卡，560px 以下轉為單欄；欄位名稱必須保留，不得只剩無標籤數值。
