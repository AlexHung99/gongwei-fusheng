...
> Schema：[schema_v1.2.sql](./schema_v1.2.sql)
> LINE Login：[line_login_v1.2.md](./line_login_v1.2.md)
> 位階種子資料：[rank_catalog_v1.2.md](./rank_catalog_v1.2.md)、[seed_rules_v1.2.sql](./seed_rules_v1.2.sql)
> NPC 初始內容：[seed_npcs_v1.2.sql](./seed_npcs_v1.2.sql)
...
v1.2 不提供人物關係分數、關係歷史、關係管理 API 或對應資料表。NPC 頁只呈現人物設定、故事、能力與公開位階資料；角色間互動結果若造成數值變化，寫入事件結果與 `character_chronicle_entries`，不得另建未列入本規格的 Relationship Endpoint。
...
{
  "id": "uuid",
  "location": { "id": "uuid", "code": "taiyi", "name": "太醫院" },
  "code": "taiyi-doctors",
  "displayName": "太醫院",
  "attendantLabel": "大夫",
...
```json
{
  ...
  "activityId": "uuid",
  "activityName": "太醫院",
  "option": { "id": "uuid", "displayName": "千澈" },
  "publicResultText": "千澈為小主診脈，體質增加 5 點。",
  "changes": [
    { "type": "stat", "code": "vitality", "before": 570, "delta": 5, "after": 575, "labelAfter": "康健" }
  ],
  "happenedAt": "2026-08-22T08:20:00Z",
  "chronicleEntryId": "uuid",
  "rulesVersion": "location-activity-v1.2"
}
```
