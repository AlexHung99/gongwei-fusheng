...
| [implementation_bootstrap_v1.1.md](./backend-spec/implementation_bootstrap_v1.1.md) | 從空白建立 Api／Admin／Worker／AdminCli、專案相依、CI 與 #20～#22 | 後端不存在時的強制開工規格 |
...
本節列的是必須建立的目標結構，不代表 Repository 已有任何專案。若 `GongWei.Api`、`GongWei.Admin` 或 `GongWei.Worker` 不存在，後端開發者必須依 [implementation_bootstrap_v1.1.md](./backend-spec/implementation_bootstrap_v1.1.md) 從空白建立並先使 Solution 通過 Build／Test；不得等待另一個專案提供骨架，也不得把「尚未建立」視為外部阻塞。
