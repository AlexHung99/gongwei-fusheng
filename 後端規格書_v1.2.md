...
| [api_v1_v1.2.md](./backend-spec/api_v1_v1.2.md) | 完整 HTTP Contract 清單 | 247 個 Method + Path、權限、DTO、錯誤與交易要求 |
...
- 新增 `location_activity_definitions`、`location_activity_options`、`location_activity_attempts`：太醫院／閱書院／奉天樓由玩家選人，太液池／御花園／上林苑由系統隨機抽籤；後台可編輯人物、獎勵、權重、排序與啟用狀態。
...
| `location_activity_definitions` | 地點選人／抽籤規則 | Select／RandomDraw、發布狀態、字數門檻、排序與版本 |
| `location_activity_options` | 大夫、先生、仙者與籤文選項 | 結構化 Reward Payload、Weight、啟用狀態；不 Hard Delete |
...
- 初始六個活動：太醫院／大夫（選擇）、閱書院／先生（選擇）、奉天樓／仙者（選擇）、太液池／宮女（隨機）、御花園／籤使（隨機）、上林苑／籤使（隨機）。初始名單與數值以 `seed_rules_v1.2.sql` 為權威來源。
- `GongWei.Admin` 新增「世界設定 → 場景人物」頁。列表可依地點、模式、發布狀態、啟用狀態搜尋；編輯頁可調整活動名稱、人物稱呼、入場文案、自戲最低字數、每日上限、獎勵說明、排序與發布狀態。
- 同頁的選項編輯器可新增／編輯／停用大夫、先生、仙者、宮女與籤使，欄位包含顯示名稱、公開獎勵文字、結構化效果、抽籤權重及排序。太醫院大夫不是寫死在前端；後台發布後由 API 即時供前端讀取。
...
- Published 活動至少須有一個 Enabled 選項；RandomDraw 的總權重必須大於 0。停用／封存只能影響新行動，既有 `location_activity_attempts`、Chronicle、Ledger、Inventory Transaction 與 Audit 永久保留。
...
- 選人及抽籤皆由 API 交易結算。RandomDraw 必須使用通過審核且達最低字數的自戲，並由伺服器抽取；玩家不可選擇籤文，前端也不可直接增加數值。
...
完整 247 支 API 在 [api_v1_v1.2.md](./backend-spec/api_v1_v1.2.md)，已包含：
...
9. Location Activity 查詢、選人與隨機抽籤交易，以及後台人物／獎勵／權重／發布管理。
...
| API Contract | 247 支 Path、Auth Policy、DTO、Problem Details、OpenAPI Snapshot |
