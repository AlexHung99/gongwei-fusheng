...
> 修訂：2026-08-16，LINE Developers Console 已登錄正式 Callback URL；Channel 暫維持 Developing
> 修訂：2026-08-22，新增六處場景人物／抽籤、後台可編輯大夫與獎勵、不可變結算及初始 Seed
> 修訂：2026-08-22，規則更正：玩家指定籤名，籤的能力與數值於後端結算完成前保密
> 狀態：v1.2 後端開發基線，可直接建立 Solution、Migration、OpenAPI 與 Sprint Backlog
...
v1.2 將領域規格拆成下列可直接開工的文件。舊版只供追溯，不得與 v1.2 混用：

| 文件 | 用途 | 規模 |
|---|---|---|
| 本文件 | 架構、領域規則、交易、狀態、開發順序 | 後端實作基準 |
| [README_v1.2.md](./backend-spec/README_v1.2.md) | 後端工程交付索引、建置順序、Bootstrap 與上線驗收 | 新環境開工入口 |
| [implementation_bootstrap_v1.2.md](./backend-spec/implementation_bootstrap_v1.2.md) | 從空白建立 Api／Admin／Worker／AdminCli、專案相依、CI 與 #20～#22 | 後端不存在時的強制開工規格 |
| [schema_v1.2.sql](./backend-spec/schema_v1.2.sql) | 可執行 PostgreSQL DDL | 63 Tables、FK、Check、Partial Unique Index、Trigger |
| [api_v1_v1.2.md](./backend-spec/api_v1_v1.2.md) | 完整 HTTP Contract 清單 | 246 個 Method + Path、權限、DTO、錯誤與交易要求 |
| [line_login_v1.2.md](./backend-spec/line_login_v1.2.md) | LINE OAuth/OIDC、PKCE、Session、CSRF、CORS 與測試矩陣 | 登入安全實作基準 |
| [rank_catalog_v1.2.md](./backend-spec/rank_catalog_v1.2.md) | 三類角色完整位號、晉降門檻、月俸與建角初值 | 規則權威來源 |
| [seed_rules_v1.2.sql](./backend-spec/seed_rules_v1.2.sql) | 可重複執行的位階、能力標籤、地圖與公開設定 Seed | 全位號與初始能力 |
| [seed_npcs_v1.2.sql](./backend-spec/seed_npcs_v1.2.sql) | 既有 8 位 NPC 初始發布資料與重繪立繪路徑 | 只新增缺少 Code，不覆蓋 CMS 編輯 |

`schema_v1.2.sql` 是資料庫結構的權威來源；EF Core Migration 產物必須與其語意一致。API 開發時需同步產生 OpenAPI，CI 比對 Breaking Change。

規格衝突時的優先順序為：本輪使用者確認事項 → 本文件的領域規則與狀態機 → `api_v1_v1.2.md` 的 HTTP Contract → `schema_v1.2.sql` 的儲存約束 → 原始 Obsidian 遊戲文件。若 DDL、OpenAPI 或程式行為與較高順位文件不同，必須先修正文檔與 Migration，不得在 Controller 內另藏例外規則。

### 0.1 v1.2 一致性修正

- LINE Developers 已建立 Provider「Max」下的 LINE Login Channel「宮闈浮生」，Channel ID `2011123657`；Console 已登錄正式 Callback `https://gongwei-api.miglow.vip/api/v1/auth/line/callback`，Channel 暫維持 Developing。Channel Secret 只能由 IIS／Secret Store 注入。
...
- 延續 `line_login_attempts`、NPC CMS、能力標籤、進度與統一歷程，並新增三張場景活動資料表，資料表總數為 63 張。
- 玩家名冊依模糊最後上線時間排序；角色今日／歷史歷程由不可變 Chronicle 統一供應，不再由前端拼湊 Ledger、Stats 與事件資料。
- 原位階文件的「主線次數」因主線模組取消，v1.2 統一映射為已完成且已結算事件次數 `settledEventCount`。
- 新增 `location_activity_definitions`、`location_activity_options`、`location_activity_attempts`：六處場景皆由玩家指定一支籤；對應能力與數值只存在後台及結算層，抽籤前不得送往玩家端。
...
本節列的是必須建立的目標結構，不代表 Repository 已有任何專案。若 `GongWei.Api`、`GongWei.Admin` 或 `GongWei.Worker` 不存在，後端開發者必須依 [implementation_bootstrap_v1.2.md](./backend-spec/implementation_bootstrap_v1.2.md) 從空白建立並先使 Solution 通過 Build／Test；不得等待另一個專案提供骨架，也不得把「尚未建立」視為外部阻塞。
...
| `location_activity_definitions` | 地點指定籤／抽籤規則 | PlayerChoiceDraw、發布狀態、字數門檻、排序與版本 |
| `location_activity_options` | 大夫、先生、仙者與籤文選項 | 隱藏的結構化 Reward Payload、啟用狀態；不 Hard Delete |
| `location_activity_attempts` | 玩家場景結算憑證 | Append-only、Idempotency、Reward Snapshot、合格自戲來源 |
...
- 初始六個活動：太醫院／大夫籤、閱書院／先生籤、奉天樓／仙者籤、太液池／宮女籤、御花園／籤使、上林苑／籤使。六處皆允許玩家指定籤名，不由伺服器改抽其他人物；初始隱藏效果以 `seed_rules_v1.2.sql` 為權威來源。
- `GongWei.Admin` 新增「世界設定 → 場景人物」頁。列表可依地點、模式、發布狀態、啟用狀態搜尋；編輯頁可調整活動名稱、人物稱呼、入場文案、自戲最低字數、每日上限、獎勵說明、排序與發布狀態。
- 同頁的選項編輯器可新增／編輯／停用大夫、先生、仙者、宮女與籤使，欄位包含顯示名稱、抽中後揭示文字、結構化能力／數值、排序與啟用狀態。太醫院大夫不是寫死在前端；玩家 API 只供籤名，隱藏效果只供後台與結算服務讀取。
- 結構化效果只接受白名單：`stat(vitality|appearance|strategy|luck|prestige, delta)` 與 `inventory(itemCode, quantity)`；不得讓管理員輸入可執行 Script、SQL、任意 CLR Type 或由 Client 傳入結算值。
- Published 活動至少須有一個 Enabled 選項。停用／封存只能影響新行動，既有 `location_activity_attempts`、Chronicle、Ledger、Inventory Transaction 與 Audit 永久保留。
- 每次後台 Create／Edit／Publish／Archive 都必填異動理由，保存 Actor、Before／After、Request ID、IP 與時間至 `audit_logs`；Audit 頁可依 `targetType=location_activity|location_activity_option` 查詢。
- 抽籤由玩家傳 `optionId` 指定籤，API 驗證歸屬及啟用狀態後才讀取隱藏效果。具有字數門檻的場景仍須使用通過審核的自戲；前端不可取得 Reward Payload 或直接增加數值。
...
完整 246 支 API 在 [api_v1_v1.2.md](./backend-spec/api_v1_v1.2.md)，已包含：

- Auth、Me、Session、CSRF。
- 玩家建角、官方立繪、人物圖片上傳／裁切／審核狀態、角色公開資料、請假與生涯。
- 管理員審核、位階、居所、狀態與死亡申請。
- 世界、地圖、六處場景人物／選人／抽籤、能力標籤、NPC 資料／故事／Revision 與公告。
...
9. Location Activity 查詢、指定籤與保密效果結算，以及後台人物／能力數值／發布管理。
...
- EF Migration 與 `schema_v1.2.sql` 約束需一致；Raw SQL Migration 建立 Partial Index、Append-only Trigger 與跨表 Trigger。
...
| API Contract | 246 支 Path、Auth Policy、DTO、Problem Details、OpenAPI Snapshot |
...
2. EF Core Migration 可從空白 PostgreSQL 建庫，並與 [schema_v1.2.sql](./backend-spec/schema_v1.2.sql) 的 FK、Check、Index、Trigger 語意一致。
3. OpenAPI 包含所有 Request／Response／Problem Details、Cookie、CSRF、Idempotency-Key、If-Match 與權限說明；實際 Path 數與 [api_v1_v1.2.md](./backend-spec/api_v1_v1.2.md) 對齊。
