...
> 版本：v0.9  
> 修訂：2026-08-15，定稿建角、位階／月俸、無限行動、投稿審核、管理調整稽核、永久保存與贊助設定  
...
> 玩法依據：[遊戲企劃規格書_v0.9.md](./遊戲企劃規格書_v0.9.md)  
> 前端依據：[前端規格書_v0.9.md](./前端規格書_v0.9.md)

---

## 0. 本版交付物

v0.9 將領域規格拆成下列可直接開工的文件：

| 文件 | 用途 | 規模 |
|---|---|---|
| 本文件 | 架構、領域規則、交易、狀態、開發順序 | 後端實作基準 |
| [schema_v0.9.sql](./backend-spec/schema_v0.9.sql) | 可執行 PostgreSQL DDL | Table、FK、Check、Partial Unique Index、Trigger |
| [api_v1_v0.9.md](./backend-spec/api_v1_v0.9.md) | 完整 HTTP Contract 清單 | 244 個 Method + Path、權限、DTO、錯誤與交易要求 |
| [rank_catalog_v0.9.md](./backend-spec/rank_catalog_v0.9.md) | 三類角色完整位號、晉降門檻、月俸與建角初值 | 規則權威來源 |
| [seed_rules_v0.9.sql](./backend-spec/seed_rules_v0.9.sql) | 可重複執行的位階與公開設定 Seed | 全位號與初始能力 |

`schema_v0.9.sql` 是資料庫結構的權威來源；EF Core Migration 產物必須與其語意一致。API 開發時需同步產生 OpenAPI，CI 比對 Breaking Change。
...
完整 244 支 API 在 [api_v1_v0.9.md](./backend-spec/api_v1_v0.9.md)，已包含：
...
7. Admin Dashboard、Audit、Approval 的死亡 Handler，以及不需覆核但強制填理由的經濟／道具異動。
...
- EF Migration 與 `schema_v0.9.sql` 約束需一致；Raw SQL Migration 建立 Partial Index、Append-only Trigger 與跨表 Trigger。
...
| API Contract | 244 支 Path、Auth Policy、DTO、Problem Details、OpenAPI Snapshot |
